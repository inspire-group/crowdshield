...
INIT_URL = "https://twitter.com/search?f=tweets&vertical=default&l=en&q={q}"
RELOAD_URL = "https://twitter.com/i/search/timeline?f=tweets&vertical=" \
             "default&include_available_features=1&include_entities=1&" \
             "reset_error_state=false&src=typd&max_position={pos}&l=en&q={q}"


def query_num_tweets(query, limit=None):
    """
    :param query: Any advanced query you want to do! Compile it at
                  https://twitter.com/search-advanced and just copy the query!
    :return:      Number of tweets fetched
    """
    ...
    try:
        while True:
            num_new_tweets, pos = query_num_on_page(
                INIT_URL.format(q=query) if pos is None
                else RELOAD_URL.format(q=query, pos=pos),
                pos is None
            )
            if len(num_new_tweets) == 0:
                return tweets

            num_tweets += num_new_tweets
            if num_tweets > curr_num + 1000:
                curr_num = num_tweets
...
def query_tweets_once(query, limit=None):
    ...
    logging.info("Got {} tweets for {}.".format(
        len(tweets), query))
    #return tweets
    return [] # Easier to keep track of who I've scraped tweets for thus far
